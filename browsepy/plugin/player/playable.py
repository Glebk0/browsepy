...
from browsepy.file import Node, File, Directory, underscore_replace, check_under_base
    ...
    def _entries(self):
        ...
        for i in range(1, self.maxsize if maxsize is None else maxsize + 1):
            ...
            yield self.playable_class(
                path=path,
                app=self.app,
                duration=parser.getint("playlist", "Length%d" % i, None),
                title=parser.get("playlist", "Title%d" % i, None),
            )
...
class M3UFile(PlayListFile):
    ...
    def _iter_lines(self):
        ...
        with codecs.open(
            self.path, "r", encoding=encoding, errors=underscore_replace
        ) as f:
