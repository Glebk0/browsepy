...
class TestPlayerBase(unittest.TestCase):
    ...
    def assertPathListEqual(self, a, b):
        return self.assertListEqual(
            list(map(os.path.normcase, a)), list(map(os.path.normcase, b))
        )
...
class TestIntegration(TestIntegrationBase):
    ...
    def test_reload(self):
        self.app.config.update(
            plugin_modules=["player"], plugin_namespaces=["browsepy.plugin"]
        )
        manager = self.manager_module.PluginManager(self.app)
        manager.load_arguments(self.non_directory_args)
        manager.reload()
...
class TestPlayable(TestIntegrationBase):
    ...
    def setUp(self):
        ...
        self.manager = self.manager_module.MimetypePluginManager(self.app)
    ...
    def test_normalize_playable_path(self):
        playable = self.module.PlayListFile(path=p(self.base, "a.m3u"), app=self.app)
        self.assertEqual(
        ...
        self.assertPathEqual(
            playable.normalize_playable_path("asdf.mp3"), self.base + "/asdf.mp3"
        )
        self.assertPathEqual(
            playable.normalize_playable_path(self.base + "/other/../asdf.mp3"),
            self.base + "/asdf.mp3",
        )
        self.assertEqual(playable.normalize_playable_path("/other/asdf.mp3"), None)

    def test_playablefile(self):
        exts = {"mp3": "mp3", "wav": "wav", "ogg": "ogg"}
    ...
    def test_playlistfile(self):
        pf = self.module.PlayListFile.from_urlpath(path="filename.m3u", app=self.app)
        self.assertTrue(isinstance(pf, self.module.M3UFile))
        pf = self.module.PlayListFile.from_urlpath(path="filename.m3u8", app=self.app)
        self.assertTrue(isinstance(pf, self.module.M3UFile))
        pf = self.module.PlayListFile.from_urlpath(path="filename.pls", app=self.app)
...
class TestBlueprint(TestPlayerBase):
    def setUp(self):
        ...
        self.app.config.update(directory_base=tempfile.mkdtemp(), SERVER_NAME="test")
    ...
    def test_endpoints(self):
        with self.app.app_context():
            self.assertIsInstance(self.module.audio(path=".."), NotFound)

            self.assertIsInstance(self.module.playlist(path=".."), NotFound)

            self.assertIsInstance(self.module.directory(path=".."), NotFound)


def p(*args):
    args = [
        arg if isinstance(arg, compat.unicode) else arg.decode("utf-8") for arg in args
    ]
