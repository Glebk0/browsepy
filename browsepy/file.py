...
codecs.register_error(
    underscore_replace, lambda error: (unicode_underscore, error.start + 1)
)
...
class File(Node):
    ...
    * :attr:`generic` is set to False, so static method :meth:`from_urlpath`
    ...
    def widgets(self):
        ...
        widgets = [
            self.plugin_manager.create_widget(
                "entry-link", "link", file=self, endpoint="open"
            )
        ]
...
class Directory(Node):
    ...
    * :attr:`generic` is set to False, so static method :meth:`from_urlpath`
    ...
    def widgets(self):
        ...
        widgets = [
            self.plugin_manager.create_widget(
                "entry-link", "link", file=self, endpoint="browse"
            )
        ]
        if self.can_upload:
            widgets.extend(
                (
                    ...
                    # ),
                    self.plugin_manager.create_widget(
                        "header", "upload", file=self, text="Upload", endpoint="upload"
                    )
                )
            )
        if self.can_download:
    ...
    def choose_filename(self, filename, attempts=999):
        ...
        if limit and limit < len(filename):
            raise FilenameTooLongError(path=self.path, filename=filename, limit=limit)
...
def check_forbidden_filename(
    filename, destiny_os=os.name, restricted_names=restricted_names
):
    """
    ...
    """
    return (
        filename in restricted_names
        or destiny_os == "nt"
        and filename.split(".", 1)[0].upper() in nt_device_names
    )
...
def check_base(path, base, os_sep=os.sep):
    ...
    :rtype: bool
    """
    return check_path(path, base, os_sep) or check_under_base(path, base, os_sep)
...
def secure_filename(path, destiny_os=os.name, fs_encoding=compat.FS_ENCODING):
    ...
    path = clean_restricted_chars(
        path,
        restricted_chars=(
            nt_restricted_chars if destiny_os == "nt" else restricted_chars
        ),
    )
...
def scandir(path, app=None):
    ...
    if exclude:
        return (item for item in compat.scandir(path) if not exclude(item.path))
