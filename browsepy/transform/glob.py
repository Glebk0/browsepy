...
class GlobTransform(StateMachine):
    ...
    def transform_posix_collating_symbol(self, data, mark, next):
        warnings.warn(
            "Posix collating symbols (like %s%s) are not supported." % (data, mark)
        )
        return None
    ...
    def transform_posix_character_class(self, data, mark, next):
        ...
        if name not in self.character_classes:
            warnings.warn("Posix character class %s is not supported." % name)
            return None
        return "".join(
            chr(start) if 1 == end - start else "%s-%s" % (chr(start), chr(end - 1))
