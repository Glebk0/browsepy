...
class TestFile(unittest.TestCase):
    ...
    def test_stats(self):
        ...
        self.assertRaises(browsepy.compat.FileNotFoundError, lambda: f.stats)
    ...
    def test_cannot_remove(self):
        ...
        self.assertRaises(self.module.OutsideRemovableBase, n.remove)

        f = self.module.File(virtual_file, app=self.app)
        self.assertFalse(f.can_remove)
        self.assertRaises(self.module.OutsideRemovableBase, f.remove)
...
class TestFileFunctions(unittest.TestCase):
    ...
    def test_secure_filename(self):
        ...
        self.assertEqual(
            self.module.secure_filename("::"), "__" if os.name == "nt" else ""
        )
        ...
        if PY_LEGACY:
            ...
            self.assertEqual(
                self.module.secure_filename("\xf1", fs_encoding="utf-8"), expected
            )
            self.assertEqual(
                self.module.secure_filename(expected, fs_encoding="utf-8"), expected
            )
        else:
            self.assertEqual(
                self.module.secure_filename("\xf1", fs_encoding="utf-8"), "\xf1"
            )
    ...
    def test_alternative_filename(self):
        ...
        self.assertEqual(
            self.module.alternative_filename("test.tar.gz", 2), "test (2).tar.gz"
        )
        self.assertEqual(
    ...
    def test_relativize_path(self):
        self.assertEqual(
            self.module.relativize_path("/parent/child", "/parent", "/"), "child"
        )
        self.assertEqual(
            self.module.relativize_path(
                "/grandpa/parent/child", "/grandpa/parent", "/"
            ),
            "child",
        )
        self.assertEqual(
