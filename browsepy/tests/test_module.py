    ...
    def from_source(cls, source, response=None):
        ...
        rows = [
            (
                ...
                any(
                    button.attrib.get("class") == "button remove"
                    for button in row[2].findall(".//a")
                ),
                any(
                    button.attrib.get("class") == "button download"
                    for button in row[2].findall(".//a")
                ),
            )
        ...
        return cls(
            ...
            all(removable for isdir, url, removable, download in rows)
            if rows
            else False,
            html.find(".//form//input[@type='file']") is not None,
            all(download for isdir, url, removable, download in rows if isdir)
            if rows
            else False,
            source,
            response,
...
class TestApp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def exclude_fnc(path):
            return path == self.exclude or path.startswith(self.exclude + os.sep)
    ...
    def get(self, endpoint, **kwargs):
        ...
        with kwargs.pop("client", None) or self.app.test_client() as client:
            response = client.get(
                self.url_for(endpoint, **kwargs), follow_redirects=follow_redirects
            )
            if response.status_code != status_code:
                raise self.page_exceptions.get(
                    response.status_code, self.page_exceptions[None]
    ...
    def post(self, endpoint, **kwargs):
        ...
        with kwargs.pop("client", None) or self.app.test_client() as client:
            response = client.post(
                self.url_for(endpoint, **kwargs), data=data, follow_redirects=True
            )
            if response.status_code != status_code:
                raise self.page_exceptions.get(
                    response.status_code, self.page_exceptions[None]
    ...
    def test_index(self):
        ...
        self.assertRaises(Page404Exception, self.get, "index")
    ...
    def test_browse(self):
        ...
        self.assertRaises(Page404Exception, self.get, "browse", path="..")

        self.assertRaises(
            Page404Exception, self.get, "browse", path="start/testfile.txt"
        )

        self.assertRaises(Page404Exception, self.get, "browse", path="exclude")
    ...
    def test_open(self):
        ...
        self.assertRaises(
            Page404Exception, self.get, "open", path="../shall_not_pass.txt"
        )
    ...
    def test_remove(self):
        ...
        self.assertRaises(
            Page404Exception, self.get, "remove", path="start/testfile.txt"
        )

        self.assertRaises(
            Page404Exception, self.post, "remove", path="start/testfile.txt"
        )

        self.app.config["directory_remove"] = None

        self.assertRaises(
            Page404Exception, self.get, "remove", path="remove/testfile.txt"
        )

        self.app.config["directory_remove"] = self.remove

        self.assertRaises(
            Page404Exception, self.get, "remove", path="../shall_not_pass.txt"
        )

        self.assertRaises(
            Page404Exception, self.get, "remove", path="exclude/testfile.txt"
        )
    ...
    def test_download_file(self):
        ...
        self.assertRaises(
            Page404Exception, self.get, "download_file", path="../shall_not_pass.txt"
        )

        self.assertRaises(Page404Exception, self.get, "download_file", path="start")

        self.assertRaises(
            Page404Exception, self.get, "download_file", path="exclude/testfile.txt"
        )
    ...
    def test_download_directory(self):
        ...
        self.assertEqual(
            tarball_files("start"), ["testfile.%s" % x for x in ("bin", "exc", "txt")]
        )

        self.app.config["exclude_fnc"] = lambda p: p.endswith(".exc")

        self.assertEqual(
            tarball_files("start"), ["testfile.%s" % x for x in ("bin", "txt")]
        )
        ...
        self.assertRaises(
            Page404Exception, self.get, "download_directory", path="exclude"
        )
    ...
    def test_upload(self):
        ...
        expected_links = sorted(
            self.url_for("open", path="upload/%s" % i) for i in files
        )
        ...
        self.assertRaises(
            ...
            "upload",
            path="start",
            data={"file": (genbytesio(127, "ascii"), "testfile.txt")},
        )
    ...
    def test_upload_restrictions(self):
        ...
        self.assertRaises(
            ...
            "upload",
            path="upload",
            data={"file": (io.BytesIO("test".encode("ascii")), longname + "a")},
        )

        subdirs = [longname] * (
            (maxpath - len(self.upload) + len(os.sep)) // (maxname + len(os.sep))
        )
    ...
    def test_sort(self):
        ...
        self.assertRaises(
            Page404Exception, self.get, "sort", property="text", path="exclude"
        )

        files = {"a.txt": "aaa", "b.png": "aa", "c.zip": "a"}
        by_name = [self.url_for("open", path=name) for name in sorted(files)]
        ...
        self.assertRaises(
            Page302Exception, self.get, "sort", property="text", client=client
        )

        page = self.get("browse", client=client)
        self.assertListEqual(page.files, by_name)

        page = self.get("sort", property="-text", client=client, follow_redirects=True)
        self.assertListEqual(page.files, by_name_desc)

        page = self.get("sort", property="type", client=client, follow_redirects=True)
        self.assertListEqual(page.files, by_type)

        page = self.get("sort", property="-type", client=client, follow_redirects=True)
        self.assertListEqual(page.files, by_type_desc)

        page = self.get("sort", property="size", client=client, follow_redirects=True)
        ...
        page = self.get(
            "sort", property="modified", client=client, follow_redirects=True
        )
    ...
    def test_endpoints(self):
        ...
        with self.app.app_context():
            self.assertIsInstance(
                self.module.sort(property="name", path=".."), NotFound
            )

            self.assertIsInstance(self.module.browse(path=".."), NotFound)

            self.assertIsInstance(self.module.open_file(path="../something"), NotFound)

            self.assertIsInstance(
                self.module.download_file(path="../something"), NotFound
            )

            self.assertIsInstance(self.module.download_directory(path=".."), NotFound)

            self.assertIsInstance(self.module.remove(path="../something"), NotFound)

            self.assertIsInstance(self.module.upload(path=".."), NotFound)
