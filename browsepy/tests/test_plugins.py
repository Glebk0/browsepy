...
class TestMimetypePluginManager(unittest.TestCase):
    ...
    def test_mimetype(self):
        ...
        self.assertEqual(manager.get_mimetype("potato"), "application/octet-stream")
        self.assertEqual(manager.get_mimetype("potato.txt"), "text/plain")
        manager.register_mimetype_function(
            lambda x: "application/xml" if x == "potato" else None
        )
        self.assertEqual(manager.get_mimetype("potato.txt"), "text/plain")
        self.assertEqual(manager.get_mimetype("potato"), "application/xml")
...
class TestPlugins(unittest.TestCase):
    ...
    def test_manager(self):
        ...
        self.assertEqual(
            endpoints, sorted(("test_x_x", "test_a_x", "test_x_a", "test_a_a"))
        )
        self.assertEqual(
            self.app.view_functions["test_plugin.root"](), "test_plugin_root"
        )
        ...
        self.assertRaises(
            self.manager_module.InvalidArgumentError, self.manager.register_widget
        )

    def test_namespace_prefix(self):
        self.assertTrue(self.manager.import_plugin(self.plugin_name))
        self.app.config["plugin_namespaces"] = (self.plugin_namespace + ".test_",)
...
def register_plugin(manager):
    ...
    manager.register_widget(
        type="button", place="entry-actions", endpoint="test_x_x", filter=lambda f: True
    )
    manager.register_widget(
        type="button",
        place="entry-actions",
