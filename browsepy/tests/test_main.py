...
class TestMain(unittest.TestCase):
    ...
    def test_params(self):
        ...
        result = self.parser.parse_args(
            ...
            + ["--plugin=%s" % plugin for plugin in plugins]
        )
        ...
        result = self.parser.parse_args(["--directory=%s" % self.base, "--initial="])
        self.assertEqual(result.host, "127.0.0.1")
        self.assertEqual(result.port, 8080)
        self.assertEqual(result.directory, self.base)
        ...
        self.assertRaises(
            SystemExit, self.parser.parse_args, ["--directory=%s" % __file__]
        )

        self.assertRaises(
            SystemExit, self.parser.parse_args, ["--exclude-from=non-existing"]
        )
    ...
    def test_exclude(self):
        ...
        match = self.module.create_exclude_fnc(result.exclude + extra, "/b", sep="/")
