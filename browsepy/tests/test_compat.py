...
class TestCompat(unittest.TestCase):
    ...
    def _warn(self, message, category=None, stacklevel=None):
        ...
        self._warnings.append(
            {"message": message, "category": category, "stacklevel": stacklevel}
        )
    ...
    def customAssertWarnsRegex(
        self, expected_warning, expected_regex, fnc, *args, **kwargs
    ):
        ...
        self.assertTrue(
            any(
                warn["category"] == expected_warning and regex.match(warn["message"])
    ...
    def test_fsencode(self):
        path = b"/a/\xc3\xb1"
        self.assertEqual(
            self.module.fsencode(path.decode("utf-8"), fs_encoding="utf-8"), path
        )
        path = b"/a/\xf1"
        self.assertEqual(
            self.module.fsencode(path.decode("latin-1"), fs_encoding="latin-1"), path
        )
        path = b"/a/\xf1"
        self.assertEqual(self.module.fsencode(path, fs_encoding="utf-8"), path)
    ...
    def test_getcwd(self):
        ...
        self.assertIsInstance(
            self.module.getcwd(fs_encoding="latin-1", cwd_fnc=lambda: b"\xf1"),
            self.module.unicode,
        )
        self.assertIsInstance(
            self.module.getcwd(fs_encoding="utf-8", cwd_fnc=lambda: b"\xc3\xb1"),
            self.module.unicode,
        )
